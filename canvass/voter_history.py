...
class voterHistoryEntry():
    ...
    def __str__(self):
        return f"{self.registration_number} voted on {self.election_date}. Absentee ballot? {self.absentee}"

    def __repr__(self):
        return f"Voter History Entry: {self.full_line}"

class voterHistoryHandler():
    voterHistoryNov2020 = dataSources.voterHistoryNov2020
    voterHistoryCompleteDirectory = dataSources.voterHistoryCompleteDirectory

    def generateVoter(self, line):
            voter = voterHistoryEntry()
            voter.full_line = line
            voter.county = county_dict[line[0:3]]
            voter.registration_number = int(line[3:11])
            voter.election_date= line[11:19]
            voter.election_type = line[19:22]
            voter.party = line[22:24]
            voter.absentee = line[24:25].upper() == "Y"
            voter.provisional = line[25:26].upper() == "Y"
            voter.supplemental = line[26:27].upper() == "Y"
            return voter

    def generate2020Voters(self, path_to_voter_history_file=dataSources.voterHistoryNov2020):
        voterHistoryFile = open(path_to_voter_history_file, 'r')
        line_count = 0
        for line in voterHistoryFile.readlines():
            yield self.generateVoter(line)
            line_count += 1
            post_updates(line_count, [1, 10, 100, 1000, 10000, 100000])
        voterHistoryFile.close()

    def getAll2020Voters(self):
        list_of_voters = []
        list_of_absentee_voters = []
        for voter in self.generate2020Voters():
            list_of_voters.append(voter.registration_number)
            if voter.absentee == True:
                list_of_absentee_voters.append(voter.registration_number)
        return list_of_voters, list_of_absentee_voters

    def generateAllVotersInDecade(self,
                                path_to_voter_history_directory=dataSources.voterHistoryCompleteDirectory):
        for root, directories, files in os.walk(path_to_voter_history_directory):
            for voter_history_file in files:
                ...
                voterHistoryFile = open(path_to_file, 'r')
                line_count = 0
                for line in voterHistoryFile.readlines():
                    yield self.generateVoter(line)
                    line_count += 1
